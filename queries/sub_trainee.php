...
if($errors == ''){
    if($in_list=="true"){
        $db->insert('trainees',array('fname'=>$fname,'lname'=>$lname,'gender'=>$gender,'cin'=>$cin,'email'=>$email,'tel'=>$tel,'institute'=>$institut,'username'=>$username,'password'=>$password,'account_state'=>0));
        $db->getResult();
